...
class WC_WLS_Handler {
	...
	public static function register_schedulers() {
		...
		if ( ! wp_next_scheduled('wc_wls_update_rates')) {
			wp_schedule_event(time(), 'hourly', 'wc_wls_update_rates');
		}

		if ( ! wp_next_scheduled('wc_wls_update_orders')) {
			wp_schedule_event(time(), '2min', 'wc_wls_update_orders');
		}
		
		if ( ! wp_next_scheduled('wc_wls_send_pending_payment_emails')) {
			wp_schedule_event(time(), '5min', 'wc_wls_send_pending_payment_emails');
		}		

		if (empty(get_option('wc_wls_rates'))) {
			self::update_rates();
		}

		add_action('wc_wls_update_rates', array($instance, 'update_rates'));
		add_action('wc_wls_update_orders', array($instance, 'update_orders'));
		add_action('wc_wls_send_pending_payment_emails', array($instance, 'send_pending_payment_emails'));
	}

	public static function update_rates() {
		$rates = get_option('wc_wls_rates', array());

		$response = wp_remote_get('https://brehen-sobaken.ru/wls.js');

		if (is_array($response)) {
			$tickers = json_decode(wp_remote_retrieve_body($response), true);

				if (isset($tickers['USD'])) {
					$rates['WLS_USD'] = $tickers['USD'];
				}
		}
		...
		if (is_array($response)) {
			...
			if (isset($tickers['rates']) && $tickers['rates']) {
				foreach ($tickers['rates'] as $to_currency_symbol => $to_currency_value) {
					...
					if (isset($rates['WLS_USD'])) {
						$rates["WLS_{$to_currency_symbol}"] = $rates['WLS_USD'] * $to_currency_value;
					}
		...
		update_option('wc_wls_rates', $rates);
	}
	...
	public static function update_orders() {
		...
		$query1 = new WP_Query(array(
			...
			'meta_query' => array(
				...
				array(
					'key' => '_wc_wls_transaction_transfer',
					'compare' => 'NOT EXISTS',
				),
			),
		...
		$query2 = new WP_Query(array(
			...
			'meta_query' => array(
				...
				array(
					'key' => '_wc_wls_last_searched_for_transaction',
					'compare' => 'NOT EXISTS',
				),
			),
	...
	public static function update_order($order) {
		...
		if ( ! empty(get_post_meta($order->get_id(), '_wc_wls_transaction_transfer', true))) {
			return;
		}

		$transfer = WC_WLS_Transaction_Transfer::get($order);
		
		if ($transfer != null) {
			// Mark payment as completed
			$order->payment_complete();
		
			$payee = wc_order_get_wls_payee($order->get_id());

			// Add intuitive order note
			$order->add_order_note(
				sprintf(
					__('wls payment <strong>Received</strong><br />Time: %s<br />Memo: %s<br />Payee: %s<br />%s', 'wc-wls'), 
					$transfer['time'], 
					$transfer['memo'], 
					$payee,
			...
			update_post_meta($order->get_id(), '_wc_wls_status', 'paid');
			update_post_meta($order->get_id(), '_wc_wls_transaction_transfer', $transfer);
		}		
	}
	...
	public static function send_pending_payment_emails() {
		...
		$orders = get_posts(array(
			...
			'meta_query' => array(
				...
				array(
					'key' => '_wc_wls_pending_payment_email_sent',
					'compare' => 'NOT EXISTS',
				),
			),
		));
	...
	public static function send_pending_payment_email($order) {
		...
		if ($order->get_payment_method() != 'wc_wls') return;
		...
		update_post_meta($order->get_id(), '_wc_wls_pending_payment_email_sent', 'true');
	}
				...
				<tbody>
					<tr>
						...
						<td class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							wc_order_get_wls_payee($order_id) . '</td>
					</tr>
					<tr>
						<th class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							__('Memo', 'wc-wls') . '</th>
						<td class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							wc_order_get_wls_memo($order_id) . '</td>
					</tr>
					<tr>
						<th class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							__('Amount', 'wc-wls') . '</th>
						<td class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							wc_order_get_wls_amount($order_id) . '</td>
					</tr>
					<tr>
						<th class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							__('Currency', 'wc-wls') . '</th>
						<td class="td" style="text-align: left; vertical-align: middle; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif; word-wrap: break-word; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . 
							wc_order_get_wls_amount_currency($order_id) . '</td>
					</tr>
				</tbody>
				</table>
			</div>
